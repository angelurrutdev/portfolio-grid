	...
	return (
		<section className='w-full grid auto-rows-[172px] grid-cols-6 gap-4'>
			...
			>
				...
				<div className='w-full flex flex-col z-[2] absolute left-0 bottom-0 leading-4 p-5 text-moonlit'>
					...
					<div className='flex flex-wrap items-center gap-2 mt-1 lg:gap-3'>
						<a
							className='flex items-center font-medium bg-moonlit px-3 py-2 rounded-lg text-dusky gap-2 active:scale-[1.1] transition-transform duration-300 hover:scale-[1.1]'
							href='https://github.com/angelurrutdev'
							target='_blank'
							rel='noreferrer'
							title='GitHub'
						>
						...
						<a
							className='flex items-center font-medium bg-moonlit px-4 py-2 rounded-lg text-dusky gap-2 active:scale-[1.1] transition-transform duration-300 hover:scale-[1.1]'
			...
			>
				...
				<MdAutoGraph className='size-full absolute -right-10 -bottom-12 opacity-5 dark:opacity-[0.02] p-5 z-[0]' />
			</motion.div>
			<motion.div
				initial={{ opacity: 0, y: 20 }}
				whileInView={{ opacity: 1, y: 0 }}
			...
			>
				<img
					className='absolute size-full object-cover z-[1]'
					src={theme === THEME_LIGHT ? whiteMap : darkMap}
					alt='Location'
					title='Location'
				/>
				<img className='w-8 z-[2]' src={logo} alt='memoji' title='memoji' />
