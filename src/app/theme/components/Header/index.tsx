...
import curriculum from '@/assets/CV_ AMUJ_2024.pdf'
	...
	return (
		<header className='w-full overflow-x-hidden h-[5rem] flex justify-center dark:bg-dusky bg-opacity-70 dark:bg-opacity-70 fixed top-0 left-0 z-10 backdrop-blur-lg subpixel-antialiased'>
			<nav className='w-full h-full flex items-center justify-between py-1 px-5 max-w-5xl xl:max-w-6xl'>
				<div className='size-7 bg-dusky flex items-center justify-center dark:bg-moonlit p-4'>
				...
				<div className='flex items-center gap-4'>
					...
					{theme === 'light' ? (
						<button title='Switch to dark theme' onClick={toggleTheme}>
							<MoonIcon className='size-6 fill-current text-[#2f3133] active:rotate-[360deg] transition-all duration-300' />
						</button>
					) : (
						<button title='Switch to white theme' onClick={toggleTheme}>
							<SunIcon className='size-6 fill-current text-sun-beam active:rotate-[360deg] transition-all duration-300' />
						</button>
					)}
					<a
			...
			{showTranslateToggle &&
				createPortal(
					<motion.div
						...
						className='size-full relative max-w-5xl px-5 xl:max-w-6xl'
					>
